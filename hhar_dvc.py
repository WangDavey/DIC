...
class HHARDataset(Dataset):
    def __init__(self, data_dir='Datasets'):
        ...
        self.data = loadmat(os.path.join(self.data_dir, 'HAR.mat'))
    
    def __getitem__(self, index):
        img = torch.tensor(self.data['X'][index]).to(torch.float32)
        lab = self.data['Y'].reshape(-1)[index]
        return img, lab
    
    def __len__(self):
        return len(self.data['X'])

class HHARDVC(BaseDVC):
    def __init__(self, entropy_weight, con_entropy_weight,seed):
        super(HHARDVC, self).__init__(
            input_dim=561,  # HHAR输入维度
            latent_dim=6,   # 潜在空间维度
            num_clusters=6,  # HHAR类别数
            dataset_name="hhar",  # 数据集名称
        ...
        self.fc4_mu = nn.Linear(2000, 6)
        self.bn4_mu = nn.BatchNorm1d(num_features=6)
        self.fc4_logvar = nn.Linear(2000, 6)
        self.bn4_logvar = nn.BatchNorm1d(num_features=6)

        # 解码器
        self.fc5 = nn.Linear(6, 2000)
        ...
        self.fc8 = nn.Linear(500, 561)
        self.bn8 = nn.BatchNorm1d(num_features=561)
        
        self.pred = nn.Linear(6, 6)
    ...
    def loss_function(self, mu, logvar, pred,recon_x,x):
        ...
        import math
        KLD_element = torch.sum(mu_v.pow(2).mul(pred_v), dim = 1)
        KLD_element = KLD_element + logvar.exp() + math.log(math.pi)
        KLD = torch.sum(KLD_element).mul_(0.5)


        f_j = torch.sum(pred, axis=0)
        entropy = torch.sum(torch.mul(f_j, torch.log(f_j+1e-8)))
