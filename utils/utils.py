...
def cluster_and_reorder(count):
    from sklearn.cluster import AgglomerativeClustering
    from scipy.cluster.hierarchy import dendrogram, linkage
    
    # 计算层次聚类的linkage矩阵
    linkage_matrix = linkage(count.T, method='ward')
    
    # 获取叶子节点的顺序
    dendrogram_dict = dendrogram(linkage_matrix, no_plot=True)
    leaf_order = dendrogram_dict['leaves']  # 这就是层次聚类的顺序
    
    # 按照层次聚类顺序重排
    reordered_count = count[:, leaf_order]
    n_col_clusters = len(np.unique(linkage_matrix[:, -1]))  # 获取聚类数
    
    return reordered_count, n_col_clusters
...
#         self.x = torch.FloatTensor(x).cuda() # 直接在初始化时移至GPU
#         self.y = torch.LongTensor(y).cuda()  # 直接在初始化时移至GPU
#         self.size = y.shape[0]
