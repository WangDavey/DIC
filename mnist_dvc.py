...
#         self.data = loadmat(os.path.join(self.data_dir, 'mnist_features_allflip_addtestp_BILINEAR.mat'))
...
class MNISTDVC(BaseDVC):
    def __init__(self, entropy_weight, con_entropy_weight,seed):
        super(MNISTDVC, self).__init__(
            input_dim=28*28,  # MNIST输入维度
            latent_dim=10,   # 潜在空间维度
            num_clusters=10,  # MNIST类别数
    ...
    def forward(self, x):
        ...
        pred = F.softmax(self.pred(z), dim=1)  # 使用z而不是mu
    ...
    def get_data_loaders(batch_size=512, data_dir='Datasets'):
        ...
        train_dataset = torchvision.datasets.MNIST(
            data_dir, transform=transform, train=True, download=True
        )
        test_dataset_all = torchvision.datasets.MNIST(
            data_dir, transform=transform, train=False, download=True
        )
